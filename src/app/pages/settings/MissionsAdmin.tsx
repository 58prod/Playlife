                ...
                <>
                    <ul className="grid grid-cols-1 gap-3">
                        {pageItems.map(mission => {
                            ...
                            return (
                                <li key={mission.id} className={`overflow-hidden rounded-2xl bg-white shadow-soft ring-1 ${mission.visible ? 'ring-ink-900/[0.06]' : 'ring-amber-300'}`}>
                                    <div className="p-4 flex flex-col lg:flex-row lg:items-center justify-between gap-4">
                                        <button type="button" onClick={() => setExpandedId(expanded ? null : mission.id)} className="flex items-center gap-2 flex-1 min-w-0 text-left" aria-expanded={expanded}>
                                            ...
                                            <span className="flex-1 min-w-0">
                                                <span className="flex items-center gap-2 flex-wrap">
                                                    <span className="font-bold text-ink-900 break-words">{mission.title}</span>
                                        ...
                                        <div className="flex flex-wrap items-center gap-2 shrink-0">
                                            ...
                                            <button type="button" onClick={() => setEditing(mission)} className="flex items-center gap-1.5 px-3 py-1.5 bg-brand-50 text-brand-500 rounded-lg text-sm font-medium hover:bg-brand-100 transition-colors">
                                    ...
                                    {expanded && (
                                        <div className="border-t border-gray-100 bg-surface-50 p-4 text-sm space-y-3">
